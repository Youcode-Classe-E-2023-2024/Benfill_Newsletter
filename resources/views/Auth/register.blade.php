...
@section("content")
    <main class="h-[900px] my-20">
        ...
        <form method="post" action="/register" class="form register-form" enctype="multipart/form-data">
            @csrf
            ...
            <label for="imageInput" class="cursor-pointer flex justify-center">
                <div class="w-16 h-16 rounded-full overflow-hidden border-2 border-gray-300">
                    <img id="previewImage" class="w-full h-full object-cover"
                         src="{{asset('/storage/image/Sample_User_Icon.png')}}" alt="User Picture">
                </div>
            </label>

            <label for="username">Username</label>
            <input class="login-input" name="name" type="text" placeholder="Username" id="username" required
                   value="{{old("name")}}">
            @error("name")
